...
public:
    ...
    Voxel(int n_x, int n_y, int n_z, Eigen::Vector3d startPoint, Eigen::Vector3d endPoint, std::vector<cv::Mat> images, std::vector<cv::Mat> silhouettes, std::vector<cv::Mat> projectionMatrices) {
...
//                    centerPointFlags[pointIdx] = inSilhouette(imagePoint, silhouettes[imageIdx]);
                    centerPointFlags[pointIdx] = visibliity(centerPoints[pointIdx], carvingThreshold);
                }
            }
//        }
    ...
    void colorRender(int colorThreshold) {
        ...
        for (int imageIdx = 0; imageIdx < images.size(); imageIdx++) {
            ...
            for (int pointIdx = 0; pointIdx < centerPoints.size(); pointIdx++) {
                ...
                if (colorConsistency(imagePoint, nextImagePoint, imageIdx, nextImageIdx, colorThreshold) && colorTemp[pointIdx](3) == 0 ) {
...
private:
    ...
    int distance(Eigen::Vector2i imagePoint1, Eigen::Vector2i imagePoint2, int imageIdx1, int imageIdx2)
    {
    ...
    bool colorConsistency(Eigen::Vector2i imagePoint1, Eigen::Vector2i imagePoint2, int imageIdx1, int imageIdx2, int threshold) {
    ...
    Eigen::Vector2i projection(cv::Mat projectionMatrix, Eigen::Vector3d centerPoint){
        ...
        double z =   projectionMatrix.at<double>(2, 0) * centerPoint.x()+
                    projectionMatrix.at<double>(2, 1) * centerPoint.y()+
                    projectionMatrix.at<double>(2, 2) * centerPoint.z() +
                    projectionMatrix.at<double>(2, 3);

        imagePoint(1) =    (projectionMatrix.at<double>(1, 0) * centerPoint.x() +
                projectionMatrix.at<double>(1, 1) * centerPoint.y() +
                projectionMatrix.at<double>(1, 2) * centerPoint.z() +
                projectionMatrix.at<double>(1, 3)) / z;

        imagePoint(0)  =    (projectionMatrix.at<double>(0, 0) * centerPoint.x() +
                projectionMatrix.at<double>(0, 1) * centerPoint.y() +
                projectionMatrix.at<double>(0, 2) * centerPoint.z() +
    ...
    bool visibliity(Eigen::Vector3d centerPoint, int carvingThreshold) {
